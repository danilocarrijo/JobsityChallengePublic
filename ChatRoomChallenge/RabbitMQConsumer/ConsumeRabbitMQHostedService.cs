...
namespace RabbitMQConsumer
{
    ...
    public class ConsumeRabbitMQHostedService : BackgroundService
    {
        ...
        public ConsumeRabbitMQHostedService(EventDispacher dispacher)
        {
        ...
        private void HandleMessage(string content)
        {
            try
            {
                if (_dispacher != null && content.Split(";").Length > 1)
                    _dispacher.RabbitMQMessage(content.Split(";")[0], content.Split(";")[1]);
            }
            catch (Exception){ }
        }
